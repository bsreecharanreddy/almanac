...
def test_result_is_sorted() -> None:
    # Shuffled input: an ascending `expected` would sort by accident and
    # dropping sorted() would stay green.
...
def test_extra_present_hours_are_ignored() -> None:
    # A present hour outside the expected range must not crash the function.
...
def test_present_counts_only_expected_hours() -> None:
    # `present` = expected hours that arrived. Counting the present set
    # directly would claim more coverage than the range holds.
...
def test_naive_hours_would_report_a_total_false_outage() -> None:
    """One naive side turns a fully-present day into a silent 24-hour outage."""
