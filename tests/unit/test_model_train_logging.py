"""log_training_run / log_classification_run: against a real local MLflow
file-store, never mocked -- the same discipline as everywhere else in
this codebase.
"""
...
def _classification_frame(n: int = 200) -> pd.DataFrame:
    rng = np.random.default_rng(2)
    x = rng.uniform(0, 10, size=n)
    return pd.DataFrame(
        {
...
def test_logs_a_classification_run_per_candidate_plus_the_baseline(tmp_path: Path) -> None:
    ...
    assert isinstance(runs, pd.DataFrame)
    # One run per candidate, plus one for the baseline.
    assert len(runs) == len(result.candidates) + 1

    by_run_name = {row["tags.mlflow.runName"]: row for _, row in runs.iterrows()}
    assert by_run_name["baseline"]["metrics.average_precision"] == pytest.approx(
        result.baseline_average_precision
    )
    for name, candidate in result.candidates.items():
        row = by_run_name[name]
        assert row["metrics.average_precision"] == pytest.approx(candidate.average_precision)
        assert row["metrics.roc_auc"] == pytest.approx(candidate.roc_auc)
        assert bool(row["metrics.is_best_candidate"]) == (name == result.best_candidate)
